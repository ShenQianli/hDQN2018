...
def train_hdqn(seed, file):
    ...
    def run_maze(max_episode):
        ...
        plt.show()
        np.savetxt("E:/my_py_project/rl/log/dsdp/hdqn/SEED_%d_avescore_log.txt" % (seed), avescore_list)
        np.savetxt("E:/my_py_project/rl/log/dsdp/hdqn/SEED_%d_testscore_log.txt" % (seed), testscore_list)
...
if __name__ == "__main__":
    ...
    print('SEED=', seed)
    file = r'E:\my_py_project\rl\log\dsdp_hdqn_score.txt'
    with open(file, 'a+') as f:
        f.write('\nSEED=' + str(seed) + '\n')
    train_hdqn(seed=seed, file=file)
